...
class DwellingUnit(Base):
    ...
    **These are permitted areas, not as-built areas.** A gramoshka records
    ...
    floor: Mapped[str | None] = mapped_column(String(20))  # text: "קרקע", "-1"
    ...
    declared_unit_count: Mapped[int | None] = mapped_column(Integer)
    is_plausible: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    plausibility_reason: Mapped[str | None] = mapped_column(Text)
    ...
    # Where in the source, e.g. "page 2, schedule row 7".
    location: Mapped[str | None] = mapped_column(Text)
    # "tesseract_regex", "openai_gpt4o_mini", or "manual".
    method: Mapped[str | None] = mapped_column(String(120))
    # The exact line as read, so a reviewer can check the reading.

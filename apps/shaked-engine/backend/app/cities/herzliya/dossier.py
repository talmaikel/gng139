...
FIELD_LABEL = {
    ...
    "tama38_event": "בקשה בתיק הבניין מזכירה תמ״א 38 או חיזוק",
...
def _scenario_caveats(assessment: dict, fields: dict, live: dict,
    ...
    if assessment.get("cap_400_sqm") and not assessment.get("cap_400_reliable"):
        ...
        # ‏W2 · הרווח מחושב על השטח לפי המדיניות, שאינו עולה על התקרה — ולכן תקרה
        # מנופחת מנפחת אותו רק כשהתקרה היא המגבלה, ותמיד את ההשוואה ל-400%.
        if post_2005 is True:
            text = ("תקרת ה-400% עשויה להיות מנופחת: בתיק הבניין יש היתר מאחרי 18.5.2005, "
                    "ולא ידוע כמה שטח הוסיף. תוספת כזו מוחרגת מהבסיס (§70ב(א)(1)(ב)), ואם "
                    "יש כזו — התקרה, ההשוואה ל-400% ותוספת הזכויות שנותרה לבקש נמוכות מהמוצג.")
        else:
            text = ("תקרת ה-400% עשויה להיות מנופחת: לא נבדק בתיק הבניין אם הותרה תוספת "
                    "בנייה אחרי 18.5.2005. תוספת כזו מוחרגת מהבסיס (§70ב(א)(1)(ב)), ואם יש "
                    "כזו — התקרה, ההשוואה ל-400% ותוספת הזכויות שנותרה לבקש נמוכות מהמוצג.")
    ...
    if cap and low and high and (low.get("share_of_cap") or 1) < 0.995:
        ...
        out.append({"id": "policy_area_unknown", "text": (
            ...
            "הרווח מחושב על כל התקרה, שהיא תקרה בחוק ולא זכות — ולכן הוא עשוי להיות גבוה בהרבה ממה שייבנה.")})
